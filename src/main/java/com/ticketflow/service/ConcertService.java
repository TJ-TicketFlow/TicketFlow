...
public class ConcertService {
    ...
    // ========================================================
    // ⚡ [새로 추가된 로직] 예매율 캐시 동기화 및 실시간 업데이트 기능
    // ========================================================
    ...
    public void refreshMainPageCache(String concertId) {
        ...
        if (concert != null) {
            ...
            // 2. 해당 공연의 현재 예매 완료된(선택된) 좌석 수 계산
            ...
            if (totalSeats > 0) {
                ...
                // [참고] 만약 Concert 엔티티 내부에 예매율(bookingRate 등)을 저장하는 컬럼과
                // setter 메서드가 명시되어 있다면 아래 주석을 해제하여 DB 데이터 자체도 동기화하세요.
                // concert.setBookingRate((float) rate);
            }
        }
